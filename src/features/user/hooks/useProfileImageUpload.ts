...
export const useProfileImageUpload = () => {
  ...
  // 폼에 저장된 현재 이미지 URL (서버에서 받은 기존 이미지)
  const currImg = useWatch({ control, name: PROFILE_FORM_KEYS.IMG_URL });
  ...
  const handleImgSelect = (file: File) => {
    setValue(PROFILE_FORM_KEYS.IMG_URL, file, { shouldValidate: true });
